...
import com.aj.model.apiface.FaceBack;
import com.aj.model.apiface.FaceParm;
...
@Slf4j
@RestController
@RequestMapping(path = "/api/face", produces = "application/json;charset=UTF-8")
public class ApiFaceController {
    ...
    public ComResp<FaceBack> compareFace(@RequestBody FaceParm faceParm) {
        ...
        if (errorCode != 0) {
            log.error("api->人脸[照片1]检测失败：" + errorCode);
        ...
        if (errorCode != 0) {
            log.error("api->人脸[照片1]特征提取失败：" + errorCode);
            resp.fail("人脸[照片1]特征提取失败！");
            return resp;
        }
        log.info("api->图片1特征：" + faceFeature.getFeatureData().length);

        byte[] byPic2 = decoder.decode(faceParm.getPic2());
        log.info("api->人脸比对[照片2]长度:" + byPic2.length);
        ImageInfo imageInfo2 = getRGBData(byPic2);
        if (imageInfo == null) {
            log.error("api->人脸比对[照片2]数据有误!");
        ...
        if (errorCode != 0) {
            log.error("api->人脸[照片2]检测失败：" + errorCode);
        ...
        if (errorCode != 0) {
            log.error("api->人脸[照片2]特征提取失败：" + errorCode);
            resp.fail("人脸[照片2]特征提取失败！");
            return resp;
        }
        log.info("api->图片2特征：" + faceFeature2.getFeatureData().length);
        ...
        if (errorCode != 0) {
            log.error("api->人脸比对失败：" + errorCode);
            resp.fail("人脸比对失败！");
            return resp;
        }
        var faceSim = faceSimilar.getScore();
        log.info("api->人脸比对相似度：" + faceSim);
